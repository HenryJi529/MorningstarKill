...
import com.morningstar.kill.domain.exception.GameCreatedFailedException;
...
import com.morningstar.kill.domain.hero.HeroPool;
...
import com.morningstar.kill.domain.player.Player;
import lombok.Data;
...
public abstract class Game {
    ...
    public String toString(){
        if(table instanceof SharedTable){
            return String.format("Game(%s, %d, %d)", id, ((SharedTable) table).getRoom1().getMemberCount(), ((SharedTable) table).getRoom2().getMemberCount());
        }else {
            return String.format("Game(%s, %d)", id, ((PrivateTable)table).getRoom().getMemberCount());
        }
    }

    public static Game createGame(Table table, Mode mode) {
        Game game;
        if(table instanceof SharedTable && (mode != Mode.TWO_VS_TWO && mode != Mode.THREE_VS_THREE)){
            throw new GameModeNotSupportedByTableTypeException();
        }
