...
### （六）市面上已有 SonarQube AI CodeFix、GitHub Copilot Autofix，为什么还要自研？

先承认：大厂也在做这个方向，恰恰说明「AI 修复静态分析发现的问题」是被验证的真实需求。关键在于现有产品解决的问题形态不同：

- **SonarQube AI CodeFix** 是 per-issue 辅助工具：对着单条 issue 点一下生成修复建议，人在 UI/IDE 里审完手动 apply——不 commit、不开 PR、没有调度、没有验证闭环。它让「人修得更快」，但技术债依旧占用开发者注意力，「人力无暇消化」的痛点没有解。
- **GitHub Copilot Autofix** 的经典模式同为逐条 suggestion；其 **agentic autofix**（公测阶段，能力仍在快速演进）形态最接近代码工坊——指派 alert 后 agent 自主探索代码库、生成修复、重跑 CodeQL 验证、自动开 PR，合并由人工裁决。

承认对手之后，真正的差异在四点：

1. **部署形态**：agentic autofix 只在 GitHub 云上运行，代码与托管必须出内网，进不了「银行内网 + Gitea/GitLab」的场景；Sonar AI CodeFix 虽可自托管（Enterprise 付费版 + 自托管 LLM 网关），但只是辅助工具而非流水线。代码工坊面向内网私有化部署，模型可插拔（国产模型 / 行内模型）。
2. **调度形态**：现有产品是「逐条指派」的功能；代码工坊是「夜间定时、多仓库、并发槽排队分发」的平台级无人值守调度——重活归 AI，人只在早上审 PR。
3. **验证深度**：agentic autofix 只有「重跑 CodeQL」一道客观门禁；代码工坊是 SonarQube 重扫门禁 + 独立语义评审两道防线，任一失败整轮回退重试。
4. **扫描通道**：CodeQL / SonarQube 都只能修规则引擎找得到的问题；代码工坊另有 AI Discovery 语义通道，覆盖竞态条件、N+1 查询、上帝类等规则外问题。

一句话：单点修复能力大家都会有，立足点在于能否部署进你的环境、以无人值守的方式跑完整条流水线。
